...
namespace PQuery
{
    public class PhysicsShape2D_Ray : PhysicsShape2D
    {
        ...
        public Vector2 GetEnd(PhysicsParameters<Vector2, RaycastHit2D, Collider2D, AdvancedOptions2D> parameters)
        {
            return GetRay(parameters).GetPoint(parameters.Distance);
        }
        public Ray2D GetRay(PhysicsParameters<Vector2, RaycastHit2D, Collider2D, AdvancedOptions2D> parameters)
        {
